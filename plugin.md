...
### 자세히 보기 — `sv-understanding` (RTL 분석가)

`sv-understanding`은 기존 RTL을 **읽고 "무엇을·왜" 하는지 산문으로 해설**하는 읽기 전용
분석가입니다(도구: `Read`/`Glob`/`Grep`/`Bash`/**`WebSearch`** — 표준·개념을 웹에서
찾아 보강). 코드를 만들거나 고치지 않습니다.

**특별한 위치 — 보조 에이전트.** 라우팅 표에서 거의 모든 작업의 **Secondary(병렬 보조)**
로 등장합니다. 즉 codegen/debug/refactor 작업 시 주 작업자와 **함께 스폰되어 "이 코드가
어떻게 동작하는지" 맥락을 제공**합니다(단독 "설명" 요청 + 다른 에이전트의 이해 파트너).

**depth/focus 프리퍼런스**로 설명을 조절:

| 설정 | 동작 |
|------|------|
| `depth: overview` / `detailed` | 고수준 블록도 / 라인별 설명 |
| `focus: fsm` | 상태·전이 설명 |
| `focus: datapath` | 신호 흐름 입력→출력 추적 |
| `focus: timing` | 파이프라인 단계·지연 |
| `focus: interface` | 포트·프로토콜 문서화 |

**맵 활용** — 코드베이스 전역 질문("X가 Y에 어떻게 연결되나")은 `.gateflow/map/CODEBASE.md`를
먼저 확인(없으면 "Run /gf-architect first"), **단일 파일**("이 모듈 설명")은 맵 없이 바로
읽어 설명. 접근법: ①맵 확인 → ②코드 먼저 읽기 → ③큰 그림 → ④세부 → ⑤ASCII 다이어그램
→ ⑥gotcha. 핵심 원칙은 **WHAT이 아니라 WHY**.

**`sv-viz`와 대비** — 둘 다 이해를 돕지만:

| | `sv-understanding` | `sv-viz` |
|---|-------------------|----------|
| 입력 | 실제 RTL 소스(맵은 보조) | `.gateflow/map/` 데이터 |
| 출력 | 산문 해설 + 부분 다이어그램 | ASCII/유니코드 다이어그램 |
| 초점 | "왜 이렇게 동작하나"(추론) | "구조가 어떻게 생겼나"(렌더) |
| 맵 | 전역 질문에만 | 항상(전제) |
